...
function Awake () {
	...
	animationComponent[die.name].layer = 5;
	
	animationComponent[gotHit.name].layer = 6;
...
function OnImHit() {
	animationComponent[gotHit.name].enabled = true;
	animationComponent.Play(gotHit.name, PlayMode.StopAll);
}

function OnDeath () {
	
	animationComponent[die.name].enabled = true;
	animationComponent.Play(die.name, PlayMode.StopAll);
	Debug.Log("I'm dying!");
}
